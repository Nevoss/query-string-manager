...
var _stringifyer = __webpack_require__(6);

var _stringifyer2 = _interopRequireDefault(_stringifyer);
...
exports.default = {
  ...
  parse: function parse() {
    ...
    _lodash2.default.forEach(str.split('&'), function (part) {
      var pair = part.replace(/\+/g, ' ').split('=');
      var value = pair[1];

      var key = _this.splitKeyIfNeeded(pair[0]);

      var pathString = '';

      for (var i = 0; i < key.length; i++) {

        if (_lodash2.default.isEmpty(key[i])) {
          var index = _lodash2.default.isArray(_lodash2.default.get(obj, _pathString)) ? _lodash2.default.get(obj, _pathString).length : 0;

          _lodash2.default.set(obj, _pathString + '[' + index + ']', value);
          continue;
        }

        var _pathString = _lodash2.default.isEmpty(_pathString) ? key[i] : _pathString + '.' + key[i];

        if (_lodash2.default.has(obj, _pathString) && !isFinite(key[i])) {
          continue;
        }

        _lodash2.default.set(obj, _pathString, value);
      }
    });
  ...
  clearString: function clearString(str) {
    return decodeURIComponent(str.trim().replace(/^[?#&]/, ''));
  },
  splitKeyIfNeeded: function splitKeyIfNeeded(key) {
